...
export default function Editor({ options, data, onOptionsChange }: any) {
  ...
  return (
    <React.Fragment>
      ...
      <Section>
        <Select
          label={t("viz.wordCloud.wordsColumn", "Words Column")}
      ...
      <Section>
        <Select
          label={t("viz.wordCloud.frequenciesColumn", "Frequencies Column")}
          ...
          <Select.Option key="none" value="">
            <i>{t("viz.wordCloud.autoCount", "(count word frequencies automatically)")}</i>
            {/* @ts-expect-error ts-migrate(2339) FIXME: Property 'Option' does not exist on type '({ class... Remove this comment to see the full error message */}
          </Select.Option>
          {map(data.columns, ({ name }) => (
            // @ts-expect-error ts-migrate(2339) FIXME: Property 'Option' does not exist on type '({ class... Remove this comment to see the full error message
      ...
      <Section>
        ...
        <ControlLabel label={t("viz.wordCloud.wordLengthLimit", "Words Length Limit")}>
          {/* @ts-expect-error ts-migrate(2322) FIXME: Type 'Element' is not assignable to type 'null | u... Remove this comment to see the full error message */}
          <Grid.Row gutter={15} type="flex">
            <Grid.Col span={12}>
              <InputNumber
                data-test="WordCloud.WordLengthLimit.Min"
                placeholder={t("viz.wordCloud.min", "Min")}
                min={0}
            ...
            <Grid.Col span={12}>
              <InputNumber
                ...
                placeholder={t("viz.wordCloud.max", "Max")}
                min={0}
      ...
      <Section>
        {/* @ts-expect-error ts-migrate(2322) FIXME: Type 'string' is not assignable to type 'null | un... Remove this comment to see the full error message */}
        <ControlLabel label={t("viz.wordCloud.wordCountLimit", "Frequencies Limit")}>
          {/* @ts-expect-error ts-migrate(2322) FIXME: Type 'Element' is not assignable to type 'null | u... Remove this comment to see the full error message */}
          <Grid.Row gutter={15} type="flex">
            <Grid.Col span={12}>
              <InputNumber
